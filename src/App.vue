<template>
  <div class='m-5'>
    <h1 class="text-5xl mb-4 font-extrabold dark:text-white">Create notes</h1>
      ...
      <div v-if="numberOfNotes">
        <button 
          type="button" 
          class="text-white bg-blue-400 dark:bg-blue-500 cursor font-medium rounded-lg text-sm px-5 py-2.5 text-center" @click="generateNotes"
          >Generate Notes
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    generateNotes() {
      axios
        .get(`http://localhost:3000/notes?count=${this.numberOfNotes}`)
        .then((response) => {
        ...
        .catch(() => {
          ...
          this.message = 'Error generating notes';
        });
